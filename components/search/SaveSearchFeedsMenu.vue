<template>
  ...
  >
    <li v-for="(feed, mainIndex) in feeds" :key="mainIndex" class="flex flex-col">
      <div
        class="w-full px-4 font-bold sticky z-1 bg-purple-midlight top-0 left-0 pt-2 whitespace-nowrap"
        :class="[classStyleName]"
      >
        <label
          class="flex space-x-2 w-full h-5 relative mt-0.5 mb-0.5 cursor-pointer"
          :for="`save-searh-${feed.id}`"
        >
          <div class="w-4 h-4 inline-block">
            <input
              :id="`save-searh-${feed.id}`"
      ...
      >
        <li v-for="(media, mediaIndex) in feed.media" :key="mediaIndex" class="flex flex-col mt-1">
          <div class="w-full px-6 font-bold whitespace-nowrap">
            <label
              class="flex space-x-2 w-full h-5 relative mt-1 mb-0.5 cursor-pointer"
              :for="`save-searh-${media.id}`"
            >
              <div class="w-4 h-4 inline-block">
                <input
                  :id="`save-searh-${media.id}`"
                  type="checkbox"
                  checked
                  class="appearance-none w-4 h-4 border-2 rounded-sm border-white toggle-check-1"
                  @click.stop="
          ...
          >
            ...
            >
              <div class="w-full px-6 font-bold whitespace-nowrap">
                <label
                  class="w-full flex space-x-2 h-5 relative mt-1 mb-0.5 cursor-pointer"
                  :for="`Save_Search_${individualMedia.value}_${individualMedia.id}`"
                >
                  <div class="w-4 h-4 inline-block">
                    <input
                      :id="`Save_Search_${individualMedia.value}_${individualMedia.id}`"
        ...
        if (mainValue === 'social_media') {
          ...
          if (mediaValue) {
            if (this.isDesktop) {
              ...
              this.feedHeight = this.feedHeight + (media.length * 28 + 2)
            }
            feedMedia.forEach((item) => {
              if (item.id === 'group') {
                item.mediaValue = false
      ...
      if (this.feeds && individualMedia) {
        ...
        if (media === 'social_media') {
          allLegends = document.querySelectorAll(
            "#save-searh-social_media input[type='checkbox']"
          )
        } else if (media === 'group') {
          allLegends = document.querySelectorAll(
            "#save-searh-group input[type='checkbox']"
          )
        } else if (media === 'name') {
            ...
            element.media.forEach((item) => {
              ...
              setTimeout(() => {
                const allLegends = document.querySelectorAll(
                  "#save-searh-all_feeds input[type='checkbox']"
                )
                allLegends.forEach((item) => {
                  if (!item.checked) {
                    item.checked = true
          ...
          if (
            ...
            item.media.forEach((element1) => {
              if (
                ...
                setTimeout(() => {
                  const allLegends =
                    document.querySelectorAll('#save-searh-group')[0]
                  // allLegends.forEach((item) => {
                  if (allLegends.checked) {
                    allLegends.checked = false
                  }
