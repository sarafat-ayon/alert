<template>
  ...
  >
    ...
    <div
      ref="area"
      :class="activeComponent === 'AreaGraph' ? 'active' : ''"
      class="
        tab
        cursor-pointer
        rounded-full
      ...
      data-index="0"
      @click="
        showTabMenu('AreaGraph', '.content__tabs', '.tab.active', $event),
          animate('.tab', $event),
          setCurrentGraph('AreaGraph')
      "
    >
      <span class="pointer-events-none xl:text-lg text-md font-bold">Area</span>
    </div>
    ...
    <div
      ...
      "
    >
      <span class="pointer-events-none xl:text-lg text-md font-bold">Line</span>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const { feedsGroupsResponseTab } = storeToRefs(store)
    onMounted(() => {
  ...
  //   },
  watch: {
    feedsGroupsResponseTab(data) {
      if (data) {
        const activeLink = this.contentWrapper.querySelector('.tab.active')
        this.tab(activeLink)
      }
    },
  },
