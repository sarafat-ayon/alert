<template>
  <section class="h-full">
    <div
      ...
      "
      :class="[showCreateGroup ? 'show md:shadow-2xl' : 'hide']"
    >
...
export default {
  ...
  computed: {
    ...mapState('setting', ['currentComponent', 'showCreateGroup']),
  ...
  methods: {
    ...
    async addNewGroup() {
      this.$v.$touch()
      if (!this.$v.$invalid) {
        this.selectedFeeds.forEach((item) => {
        ...
        if (this.group.feedsInGroup.length > 0) {
          try {
            ...
            const res = await this.$axios.$post(GROUPS, this.group)
            if (res.success) {
              this.close = false
              this.addGroup = true
        ...
        } else if (this.availableFeeds.length > 0) {
