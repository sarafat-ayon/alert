<template>
  <div class="flex flex-row w-full">
    ...
    <div class="w-full">
      <div class="bg-offwhite-200 p-3 rounded-sm w-full">
        <div class="break-words">
          <div class="flex flex-wrap justify-between w-full">
            <h2 v-if="comment.name" class="text-ash-primary text-md">
              {{ comment.name }}
          ...
          <div v-if="comment.userName" class="text-gray-1700">
            {{ comment.userName }}
          </div>
          <div
            v-if="comment.text"
            v-see-more="200"
            class="text-ash-primary text-md"
            :class="[comment.userName && comment.name ? 'mt-3' : '']"
            v-html="comment.text"
          ></div>
      ...
      <div class="w-full lg:px-1 md:px-0 px-1 min-w-max">
...
export default defineComponent({
  name: 'LinkedInReply',
  components: { linkedInTube, LinkedInLike },
  props: {
    comment: {
      type: Object,
      default: null,
