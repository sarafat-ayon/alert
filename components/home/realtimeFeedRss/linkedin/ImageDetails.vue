      ...
      >
        ...
        <transition name="imagezoom">
          <div id="zoom" ref="zoom" class="overflow-hidden w-full zoom">
            ...
            @mousemove="mouseMove" -->
            <img
              v-if="singlePost.mediaUrls.length === 1"
              id="zoomContainer"
              ref="objImage"
              :src="singlePost.mediaUrls[0]"
              alt="Image not found"
              class="
                mt-0
                object-contain
            ...
            >
              <slide
                v-for="(carouselImage, sIndex) in singlePost.mediaUrls"
                :key="sIndex"
                class="
            ...
            <carousel
              v-if="singlePost.mediaUrls.length > 1 && show && !showFullImage"
              :autoplay="false"
              autoplay-direction="forward"
              autoplay-hover-pause
              :autoplay-timeout="2000"
            ...
            <carousel
              v-else-if="
                singlePost.mediaUrls.length > 1 && !show && showFullImage
              "
              :autoplay="false"
              autoplay-direction="forward"
              autoplay-hover-pause
        ...
        >
          <div ref="desktopBody" class="overflow-hidden max-h-full h-full">
            <div ref="contentHeight" class="max-h-full bg-white h-full">
              ...
              <div id="post-card" class="scroll h-1/2 linkedin-image-card mb-3">
                <div class="card scroll h-full">
                  <div class="flex flex-row justify-between w-full px-0 pt-0">
                  ...
                  <div class="w-full">
                    ...
                    >
                      ...
                      <span class="mx-px font-normal text-xxs text-gray-1700"
                        >•</span
                      >
                      <span class="text-gray-1700 cursor-pointer"
...
export default defineComponent({
  ...
  methods: {
    ...
    defineCardHeight() {
      setTimeout(() => {
        const card = document.querySelectorAll('.linkedin-image-card')
        card.forEach((item) => {
    ...
    heightInitial() {
      if (this.singlePost.mediaUrls.length > 1) {
        setTimeout(() => {
      ...
      if (this.singlePost.mediaUrls.length > 1) {
        ...
        display1.forEach((item) => {
          if (item.classList.contains('VueCarousel-slide-active')) {
            ...
            display1[
              display1.length - this.singlePost.mediaUrls.length + this.index
...
.card {
  @apply bg-white p-4 md:pt-3 my-0 flex flex-col;
